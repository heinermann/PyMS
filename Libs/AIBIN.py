			...
			while data[offset:offset+4] != b'\x00\x00\x00\x00':
				...
				if string > len(self.tbl.strings):
					if addstrings:
						...
						warnings.append(PyMSWarning('Load',f"The AI with ID '{id}' had a custom string (#{string}) not present in the stat_txt.tbl, so PyAI generated one.", level=1))
					else:
...
#<0>
	...
	def interpret(self, files, defs=None, extra=False):
			...
			try:
				...
				if d.lower() in variables:
					for pt in self.typescanbe[p.__doc__.split(' ',1)[0]]:
						...
						raise PyMSError('Variable',"Incorrect type on varaible '{}'. Excpected '{}' but got '{}'".format(d.lower(), p.__doc__.split(' ',1)[0], variables[d.lower()][0][0].__doc__.split(' ',1)[0]),n,line, warnings=warnings)
					var = PyMSWarning('Variable',"The variable '{}' of type '{}' was set to '{}'".format(d, variables[d.lower()][0][0].__doc__.split(' ',1)[0], variables[d.lower()][1]))
				return p(da,3)
				...
				raise PyMSError('Parameter',"Invalid parameter data '{}', looking for type '{}'".format(d,p.__doc__.split(' ',1)[0]),n,line, warnings=warnings)
					...
					if line:
						...
						if match:
							...
							if re.match('[\x00,(){}]',name):
								raise PyMSError('External Definition',f"The variable name '{name}' in external definition file '{defname}' is invalid, it must not contain a null, or the characters: , ( ) {{ }}",n,line, warnings=warnings)
							t = t.lower()
							if t == 'label' or t not in self.types:
								raise PyMSError('External Definition',f"Invalid variable type '{t}' for variable '{name}' in external definition file '{defname}', consult the reference",n,line, warnings=warnings)
							if name.lower() in variables:
								raise PyMSError('External Definition',f"The variable name '{name}' in external definition file '{defname}' is already in use",n,line, warnings=warnings)
							if vinfo:
							...
							try:
								...
								raise PyMSError('External Definition',f"Invalid value '{dat}' for variable '{name}' of type '{t}' in external definition file '{defname}'",n,line, warnings=warnings)
						...
						if multiline:
							...
							if match:
								...
								try:
									...
									raise PyMSError('Interpreting',f"Invalid variable value '{dat}' for type '{t}'",n,line, warnings=warnings)
								variables[name.lower()] = [self.types[t],dat]
							...
							if re.match('\\A[^(]+\\([^)]+\\):\\s*(?:\\{.+\\})?\\Z', line):
								...
								if ai:
									...
									if None in ai[5]:
										...
										if isstr(dat):
											raise PyMSError('Interpreting',f"There is no block with name '{dat}' in AI with ID '{ai[0]}'", warnings=warnings)
									if ai[0] in findtotaljumps and findtotaljumps[ai[0]]:
										n = list(findtotaljumps[ai[0]].keys())[0]
										raise PyMSError('Interpreting',f"There is no block with name '{n}' in AI with ID '{ai[0]}'", warnings=warnings)
												...
												if match:
													...
													if cid in totaljumps:
														...
														if cid in scriptids[0]:
															...
															b = 'bwscript'
														if a != b:
															raise PyMSError('Interpreting',f"You can't jump to an external script in '{b}.bin' from a script in '{a}.bin'",n,line, warnings=warnings)
														if not label in totaljumps[cid]:
															raise PyMSError('Interpreting',f"The AI with ID '{cid}' has no label '{label}'", warnings=warnings)
									...
									if ai[0] in findtotaljumps and label in findtotaljumps[ai[0]]:
										...
										for fj in findtotaljumps[ai[0]][label]:
											if fj[4] in scriptids[0]:
												...
												b = 'bwscript'
											if a != b:
												raise PyMSError('Interpreting',f"You can't jump to an external script in '{a}.bin' from a script in '{b}.bin'",n,line, warnings=warnings)
		...
		if findtotaljumps:
			...
			l = i[1].peek()
			raise PyMSError('Interpreting',f"The external jump '{i[0]}:{l[0]}' in AI script '{l[1][0][4]}' jumps to an AI script that was not found while interpreting (you must include the scripts for all external jumps)", warnings=warnings)
		s = 2+sum(aisizes.values())
		if s > 65535:
			raise PyMSError('Interpreting',f"There is not enough room in your aiscript.bin to compile these changes. The current file is {2+sum(self.aisizes.values())}B out of the max 65535B, these changes would make the file {s}B.")
		s = 2+sum(bwsizes.values())
		if s > 65535:
			raise PyMSError('Interpreting',f"There is not enough room in your bwscript.bin to compile these changes. The current file is {2+sum(self.bwsizes.values())}B out of the max 65535B, these changes would make the file {s}B.")
		if findgoto:
			remove = [{},{}]
			for i in findgoto.items():
				...
				for l,f in i[1].items():
					if not f[0]:
						add_warning(PyMSWarning('Interpeting',f"The label '{l}' in AI script '{i[0]}' is unused, label is discarded"))
		...
		for id,u in unused.items():
			if u and (not id[0] in findgoto or not id[1] in findgoto[id[0]]):
				add_warning(PyMSWarning('Interpeting',f"The label '{id[1]}' in AI script '{id[0]}' is only referenced by commands that cannot be reached and is therefore unused"))
		if self.ais:
	...
	def decompile(self, file, defs=None, ref=False, shortlabel=True, scripts=None):
					...
					if len(l) > 1:
						...
						if line:
							...
							if match:
								t,name,dat,vinfo = match.groups()
								if re.match('[\x00,(){}]',name):
									raise PyMSError('External Definition',f"The variable name '{name}' in external definition file '{dname}' is invalid, it must not contain a null, or the characters: , ( ) {{ }}",n,line, warnings=warnings)
								t = t.lower()
								if t == 'label' or t not in self.types:
									raise PyMSError('External Definition',f"Invalid variable type '{t}' for variable '{name}' in external definition file '{dname}', consult the reference",n,line, warnings=warnings)
								if name.lower() in variables:
									raise PyMSError('External Definition',f"The variable name '{name}' in external definition file '{dname}' is already in use",n,line, warnings=warnings)
								if vinfo:
								...
								try:
									...
									raise PyMSError('External Definition',f"Invalid value '{dat}' for variable '{name}' of type '{t}' in external definition file '{dname}'",n,line, warnings=warnings)
								variables[name.lower()] = [self.types[t],dat]
		...
		for id in scripts:
			...
			if loc:
					...
					if cmdn in jump:
						...
						if id in self.externaljumps[0][0] and cmdn in self.externaljumps[0][0][id]:
							...
							if len(self.externaljumps[0][0][id][cmdn]) > 1:
								s = 's'
							f.write('\t\t# Referenced externally by script{}: {}\n'.format(s, ', '.join(self.externaljumps[0][0][id][cmdn])))
						f.write('\t\t--%s--' % jump[cmdn])
							...
							if t == self.ai_address:
								if type(temp[1]) == list:
									if temp[1][0] in extjumps and temp[1][1] in extjumps[temp[1][0]]:
										f.write(f'{temp[1][0]}:{extjumps[temp[1][0]][temp[1][1]]}')
									else:
				...
				f.write(f'# stat_txt.tbl entry {string}: {TBL.decompile_string(self.tbl.strings[string])}\n{id}({string}, {convflags(flags)}, bwscript):')
				labelnames = None
	...
	def compile(self, file, bwscript=None, extra=False):
		...
		f.write('{}{}{}\x00\x00\x00\x00'.format(struct.pack('<L', offset),ais,table))
		if extra and (self.varinfo or self.aiinfo):
			info = ''
			for var,dat in self.varinfo.items():
				if dat[2] != None:
					info += f'{chr(dat[0]+1)}{var}\x00{self.types[types[dat[0]]][0](dat[1],2)[1]}{dat[2]}\x00'
			info += '\x00'
			for ai,dat in self.aiinfo.items():
				info += f'{ai}{dat[0]}\x00'
				for label,desc in dat[1].items():
					info += f'{label}\x00{desc}\x00'
				info += '\x00'
				if dat[2]:
				...
				if cmdn in jump:
					...
					if id in self.externaljumps[1][0] and cmdn in self.externaljumps[1][0][id]:
						...
						if len(self.externaljumps[1][0][id][cmdn]) > 1:
							s = 's'
						f.write('\t\t# Referenced externally by script{}: {}\n'.format(s, ', '.join(self.externaljumps[1][0][id][cmdn])))
					f.write('\t\t--%s--' % jump[cmdn])
					if labelnames:
						i = labelnames.get(jump[cmdn],'')
						if '\n' in i:
