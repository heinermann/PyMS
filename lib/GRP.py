...
def frame_to_photo(p, g, f=None, buffered=False, size=True, trans=True, transindex=0, flipHor=False, draw_function=None, draw_info=None):
	...
	pal = list(map(lambda i,p=p,e=draw_info: draw_function(p,i,e), range(len(p))))
	pal[transindex] = (0,0,0,0)
	if size:
		image = [None,-1,-1,-1,-1]
					...
					for offset in offsets:
						...
						while len(linedata)-xoffset < linewidth:
							...
							if o & 0x80:
								...
								linedata.extend(self.databuffer[offset+1:offset+1+o])
								offset += o + 1
			...
			if self.uncompressed:
				try:
					for offset in offsets:
						...
						if xoffset > 0:
							linedata = [0] * xoffset
						linedata.extend([index for index in self.databuffer[offset:offset+linewidth]])
				...
				for frame in range(frames):
					...
					if uncompressed:
						for line in range(lines):
							...
							linedata.extend([index for index in data[framedata:framedata+linewidth]])
							...
							for line in range(lines):
								...
								while len(linedata)-xoffset < linewidth:
									...
									if o & 0x80:
										...
										linedata.extend([c for c in data[offset+1:offset+1+o]])
										offset += o + 1
